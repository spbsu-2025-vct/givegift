...
* `interest` - таблица предлагаемых пользователю интересов. При генерации идей подарков, интересы человека выбираются именно оттуда _(сделал я это намеренно, т.к. если пользователь напишет что-то не то, это может сломать промпт, и LLM вероятно сгаллюционирует)_
...
4. `PUT /favourites/edit_tag` - изменяет тег товара в избранных
   ...
   В теле запроса:
   * `favProduct` - объект, описывающий товар
   * `newTag` - тег, на который хочется поменять
