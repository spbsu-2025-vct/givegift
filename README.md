...
И главное - вы сразу увидите удобный подбор с прямыми ссылками на маркетплейсы для быстрой и лёгкой покупки _(в рамках данного проекта выбран Wildberries)_!
...
* `interest` - таблица предлагаемых пользователю интересов. При генерации идей подарков, интересы человека выбираются именно оттуда _(сделал я это специально, т.к. если пользователь напишет что-то не то, это может сломать промпт, и LLM вероятно сгаллюционирует)_
...
#### TODO размышления насчет bottleneck'ов

#### Структура приложения
Рассмотрим с высоты птичьего полета на схему работы приложения
TODO
...
4. `PUT /favourites/edit_tag` - изменяет тег товара в избранных

   В теле запроса:
   * Объект, описывающий товар
   * Тег, на который хочется поменять
   ...
   ```json
   {
     "favProduct": {
         "userID": "f4552192-1a6d-4b76-9ce4-784424e6189d",
         "tag": "Привет!!",
         "market_link": "https://www.wildberries.ru/catalog/191657254/detail.aspx",
         "title": "Невские просторы Стихи",
         "img_link": "https://basket-12.wbbasket.ru/vol1916/part191657/191657254/images/big/1.webp"
     },
     "newTag": "Привет!!!"
   }
   ```
...
Есть админка на базе AdminJS с базовой аутентификацией. Для этого нужно зайти по адресу `/admin`
