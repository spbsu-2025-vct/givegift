...
* На данный момент WB Parser держится внутри API, т.к. это один небольшой файлик, и смысла в его отделении сейчас нет. Но при росте проекта может будет уже не один маркетплейс, и тогда уже можно выделить отдельный сервис Parsing Service _(я поэтому отдельно показываю это на схеме)_
